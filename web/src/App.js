...
class App extends Component {
  constructor(props) {
    super(props)
    this.state = { loggedIn: false, tripType: "returntrip", account: {}, inputClassName: "column col-3", resultsMounted: true, searched: false }
  }
  componentDidMount() {
  ...
  setTripType = (e) => {
    if (e.target.id === "oneway") {
      ...
      this.setState({ inputClassName: "column col-4" })
    }
    else {
      this.setState({ inputClassName: "column col-3" })
    }
  ...
  login = async (user, pass) => {
    ...
    await dataFacade.login(user, pass)
      .then(res => {
  ...
  fetchFlights = async (e) => {
    e.preventDefault();
    const { departureLoc, arrivalLoc, dateDeparture, dateReturn } = this.state;
    if (departureLoc && arrivalLoc) {

      this.setState({ resultsMounted: true, searched: true })
      if (this.state.tripType === "returntrip") {
        const flights = await dataFacade.getReturnRoutes(departureLoc.toUpperCase(), arrivalLoc.toUpperCase(), dateDeparture, dateReturn)
        ...
        <div className="sp-container off-canvas off-canvas-sidebar-show container">
          ...
          <Router>
            <>
              ...
              <Route exact path="/" render={() => <Home state={this.state} onDataChanged={this.onDataChanged} removeArrivalDate={this.removeReturnDate} setTripType={this.setTripType} tripType={this.state.tripType} inputClassName={this.state.inputClassName} fetchFlights={this.fetchFlights} resultsMounted={this.state.resultsMounted} searched={this.state.searched} flights={this.state.flights} sortedFlights={this.state.sortedFlights} filterList={this.filterList} />} />
...
const Home = (props) => {
  return (
    <>
      <SearchParameters state={props.state} onDataChanged={props.onDataChanged} removeArrivalDate={props.removeArrivalDate} tripType={props.tripType} setTripType={props.setTripType} inputClassName={props.inputClassName} fetchFlights={props.fetchFlights} resultsMounted={props.resultsMounted} searched={props.searched} flights={props.flights} sortedFlights={props.sortedFlights} filterList={props.filterList} />
    </>
  );
}
        ...
        <section className="navbar-section">
          ...
          <div className="sp-brand">
            <Link className="sp-logo" to='/'>
              <img src={logo} alt="Spectre.css CSS Framework" /></Link>
          </div>
        </section>
